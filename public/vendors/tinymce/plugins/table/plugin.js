...
(function () {
  ...
  var getLockedColumnsFromTable = function (table) {
    return getOpt(table, LOCKED_COL_ATTR)
      .bind(function (lockedColStr) {
  ...
  var adjustAndRedistributeWidths = function (
    _table,
    list,
    details,
    tableSize,
    ...
    var _a = resizeBehaviour.calcRedestributedWidths(
        sizes,
        tablePixelWidth,
        details.pixelDelta,
        tableSize.isRelative
  ...
  var resizeTable = function () {
    ...
    var calcRedestributedWidths = function (
      sizes,
      totalWidth,
      pixelDelta,
      isRelative
      ...
      each(grid, function (row) {
        ...
        if (isToReplace) {
          mutateCell(
            row,
            index,
            elementnew(substitution(), true, current.isLocked)
          );
        }
      });
    }
    ...
    for (var r = startRow; r < endRow; r++) {
      ...
      for (var c = startCol; c < endCol; c++) {
        ...
        mutateCell(
          gridA[r],
          c,
          elementnew(replacement, true, newCell.isLocked)
        );
      }
    }
    return gridA;
  };
  var getValidStartAddress = function (
    currentStartAddress,
    grid,
    lockedColumns
  ) {
    var gridColLength = cellLength(grid[0]);
    var possibleColAddresses = range(
  ...
  var getLockedColumnsWithinBounds = function (
    startAddress,
    grid,
    lockedColumns
  ) {
    return filter(lockedColumns, function (colNum) {
      return (
        ...
        colNum <= cellLength(grid[0]) + startAddress.column
      );
    });
  };
  var merge$1 = function (startAddress, gridA, gridB, generator, comparator) {
    var lockedColumns = getLockedColumnsFromGrid(gridA);
    var validStartAddress = getValidStartAddress(
      startAddress,
      gridA,
      lockedColumns
    );
    var lockedColumnsWithinBounds = getLockedColumnsWithinBounds(
      validStartAddress,
      gridB,
      lockedColumns
    );
    ...
    return result.map(function (diff) {
      ...
      var newLockedColumnsWithinBounds = getLockedColumnsWithinBounds(
        validStartAddress,
        gridB,
        newLockedColumns
      );
  ...
  var deleteColumnsAt = function (grid, columns) {
    return bind(grid, function (row) {
      ...
      var cells = foldr(
        columns,
        function (acc, column) {
          return column >= 0 && column < acc.length
    ...
    return map(grid, function (row) {
      return mapCells(row, function (cell) {
        return isTarget(cell)
          ? elementnew(
              substitution(cell.element, comparator),
              true,
              cell.isLocked
            )
          : cell;
  ...
  var opInsertColumnsAfter = function (
    grid,
    extractDetail,
    comparator,
    genWrappers
  ...
  var insertColumnsExtractor = function (before) {
    return function (warehouse, target) {
      return onCells(warehouse, target)
  ...
  var TableActions = function (editor, lazyWire, selections) {
    ...
    var execute = function (operation, guard, mutate, lazyWire, effect) {
      return function (table, target) {
        ...
        var resizeBehaviour = isResizeTableColumnResizing(editor)
          ? resizeTable()
          : preserveTable();
        return guard(table)
          ? operation(
    ...
    var insertRowsBefore$1 = execute(
      insertRowsBefore,
      always,
      noop,
      lazyWire,
      ...
      .bind(function (selectedCells) {
        ...
        var filteredGrid = bind(slicedGrid, function (row) {
          ...
          return newCells.length > 0
            ? [__assign(__assign({}, row), { cells: newCells })]
            : [];
        });
  ...
  var applyCellData = function (editor, cells, oldData, data) {
    ...
    if (size(modifiedData) > 0 && cells.length >= 1) {
      ...
      var styleModified_1 =
        size(
          filter$1(modifiedData, function (_value, key) {
